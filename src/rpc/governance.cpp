...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in encocoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("encocoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Encocoin features */
    { "encocoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "encocoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "encocoin",               "gobject",                &gobject,                true,  {} },
    { "encocoin",               "voteraw",                &voteraw,                true,  {} },
