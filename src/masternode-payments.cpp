...
*
*   Why is this needed?
*   - In Encocoin some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Encocoin specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f XNK\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
