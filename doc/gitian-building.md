...
*Setup instructions for a Gitian build of Encocoin Core using a Debian VM or physical system.*
...
Clone the git repositories for Encocoin Core and Gitian.
...
in the Encocoin Core repository under 'Fetch and create inputs' to install sources which require
...
To build Encocoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Encocoin Core repository.
...
```bash
    ...
    From https://github.com/encocoinpay/encocoin
...
and the encocoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../encocoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/encocoin
SIGPATH=/some/root/path/encocoin-detached-sigs

./bin/gbuild --url encocoin=${BTCPATH},signature=${SIGPATH} ../encocoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/encocoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/encocoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/encocoin-osx-build.assert
```
...
[encocoin/gitian.sigs](https://github.com/encocoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
